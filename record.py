...
class EdgeImpulseUploader:
    ...
    def get_inference_result(self, audio_filename):
        try:
            with open(audio_filename, "rb") as f:
                response = requests.post(
                    self.inference_url,
                ...
                if response.status_code == 200:
                    return response.json()  # Return inference result as JSON
                else:
                    QtWidgets.QMessageBox.warning(
                        None, "Inference Failed", 
                        f"Failed with status code: {response.status_code}\nResponse: {response.text}")
                    return None
        except requests.exceptions.RequestException as e:
            QtWidgets.QMessageBox.critical(None, "Inference Error", f"Error occurred: {e}")
            return None
...
class SignalRecorder(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.infer_button = QtWidgets.QPushButton("Run Inference")
        ...
        main_layout.addWidget(self.infer_button)
    ...
    def run_inference(self):
        """Run inference on the recorded audio and display the result."""
        file_path = self.save_audio()
        result = self.uploader.get_inference_result(file_path)
        if result:
            self.show_inference_result(result)

    def show_inference_result(self, result):
        """Display the inference result."""
        if "result" in result:
            results_text = "Inference Results:\n"
            for label, value in result["result"].items():
                results_text += f"{label}: {value:.2f}\n"
            QtWidgets.QMessageBox.information(None, "Inference Results", results_text)
        else:
            QtWidgets.QMessageBox.warning(None, "Error", "No results found in inference response.")
